...
class CandidateApp(MayanAppConfig):
    app_namespace = 'candidate'
    app_url = 'candidate'
    has_rest_api = True
    has_tests = True
    ...
    def ready(self):
        ...
        CandidateReview = self.get_model(model_name='CandidateReview')

        EventModelRegistry.register(model=Candidate)
        EventModelRegistry.register(model=CandidateReview)

        # ModelCopy(
        #     model=Candidate, bind_link=True, register_permission=True
        # ).add_fields(
        #     field_names=(
